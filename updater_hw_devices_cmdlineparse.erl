...
valid_options() ->
    [#options{type=opt, short="-g", long="--background"   ,map_field=background,                 map_value=true},
     ...
     #options{type=opt, short="-P", long="--port"         ,map_field=port,                       map_value=sub_parameter},
     #options{type=opt, short="-k", long="--part-number"  ,map_field=part_number,                map_value=sub_parameter},
     #options{type=opt, short="-T", long="--cfgtype"      ,map_field=cfgtype,                    map_value=sub_parameter},
     #options{type=cmd, short="-h", long="--help"         ,map_field=command,                    map_value=showhelp},
...
show_help(CfgFileName, Board_type, Active, LocalPartNumber) ->
    ...
    io:format("    -P, --port=<port>                         = specify a new port file~n"),
    io:format("    -T, --cfgtype=<.cfg or .xml>              = specify the cfg file typee~n"),
    io:format("    -k, --part-number=<part-number>           = specify a 2ek board part number~n"),
    io:format("~n"),
